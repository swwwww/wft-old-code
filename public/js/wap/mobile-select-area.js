	...
	MobileSelectArea.prototype = {
		...
		bindEvent: function() {
			...
			this.trigger[_this.settings.eventName](function(e) {
				...
				_this.scroller.children().bind('touchstart', function(e) { //bind-->on
					start = (e.changedTouches || e.originalEvent.changedTouches)[0].pageY;
				});
				_this.scroller.children().bind('touchmove', function(e) {//bind-->on
				...
				//add
				_this.scroller.on('touchmove', function (e) {
					e.preventDefault();
				});


				_this.scroller.children().bind('touchend', function(e) {//bind-->on
					end = (e.changedTouches || e.originalEvent.changedTouches)[0].pageY;
					var diff = end - start;
					var dl = $(e.target).parent();
					if (dl[0].nodeName != "DL") {
		...
		format: function() {
			...
			this.f(this.data);
			// console.log(_this);
			console.log(_this.text.join(" "))
			 $('#txt_area').val(_this.text.join(" "));
		...
		submit: function() {
			...
			this.trigger.next('input[type="hidden"]').val(this.value.join(','));
