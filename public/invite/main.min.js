...
window.Zepto = Zepto, void 0 === window.$ && (window.$ = Zepto), function (a) {
    ...
    }, q = {}, r = {}, s = "onfocusin"in window, t = {focus: "focusin", blur: "focusout"}, u = {
    ...
    function a() {
        ...
        //if ("" == a)return o.getElementsByTagName("i")[0].innerHTML = "您还未输入手机号码哦", j.show("text"), !1;
        //if (!m.test(a))return o.getElementsByTagName("i")[0].innerHTML = "您输入的手机号码有误", j.show("text"), !1;
        n = !0, l.innerHTML = "领取中...";
        var b = "/webinvite/index/bind";
        $.ajax({
            type: "POST", dataType: "json", url: b, data: {phone: a, token : token,city:city,orcity:ocity}, success: function (res) {
                //console.log(res);
                if(res.code > 0){
                    window.location.href = '/webinvite/index/result?phone='+a+'&token='+token;
                }else{
                    alert(res.msg);
                    location.reload();
                }
            }, error: function () {
                l.classList.add("check-disabled"), l.innerHTML = "领取失败", n = !0
            }
        })
    }

    var b = document.body, c = navigator.userAgent.toLowerCase(), d = "touchstart", e = "touchmove", f = "touchend", g = "touchend", h = document.getElementsByClassName("check-rule")[0], i = document.getElementsByClassName("check-code")[0];
    c.match("mobile") || (d = "mousedown", e = "mousemove", f = "mouseup", g = "click");
    var j = new function () {
        var a, c = document.getElementById("check-win"), d = document.getElementById("check-win-box"), e = document.getElementsByClassName("check-win-list"), f = document.getElementsByClassName("check-win-rule")[0], h = document.getElementsByClassName("check-win-code")[0], i = document.getElementsByClassName("check-win-tips")[0], k = document.getElementsByClassName("check-win-remind"), l = document.getElementsByClassName("check-win-send"), over = document.getElementsByClassName("check-win-over"), unavailable = document.getElementsByClassName("check-win-unavailable"), m = function () {
            c.style.background = "rgba(0,0,0,0)", d.style.cssText = "-webkit-transform:translate(-50%,-50%) scale(0.4);transform:translate(-50%,-50%) scale(0.4);opacity:0;", a = setTimeout(function () {
                c.style.display = "none", b.removeEventListener("touchmove", noMove, !1)
            }, 400)
        };
        this.show = function (g, j) {
            clearTimeout(a);
            for (var m = 0; m < e.length; m++)e[m].style.display = "none";
            //console.log(g);
            switch (b.addEventListener("touchmove", noMove, !1), g) {
                case"rule":
                ...
                case"text":
                    ...
                    break;
                case"remind":
                    for (var n = 0; n < k.length; n++)k[n].style.display = "block";
                    break;
                case"send":
                    for (var o = 0; o < l.length; o++)l[o].style.display = "block";
                    break;
                case"over":
                    for (var ov = 0; ov < over.length; ov++)over[ov].style.display = "block";
                    break;
                case"unavailable":
                    for (var ava = 0; ava < unavailable.length; ava++)unavailable[ava].style.display = "block";
                    break;
            }
            c.style.display = "block", a = setTimeout(function () {
                c.style.background = "rgba(0,0,0,0.8)", d.style.cssText = "-webkit-transform:translate(-50%,-50%) scale(1);transform:translate(-50%,-50%) scale(1);opacity:1;"
            }, 100)
        }, this.hide = m, d.addEventListener(g, function (a) {
            a.target && a.target.classList.contains("check-win-close") && j.hide()
        }, !1)
    };
    void 0 != h && h.addEventListener(g, function () {
        j.show("rule")
    }, !1), void 0 != i && i.addEventListener(g, function () {
        j.show("code")
    }, !1);
    var k = document.getElementById("phone"), tk = document.getElementById("token"), ct = document.getElementById("city"),oct = document.getElementById("orcity"), l = document.getElementById("submitBtn"), m = /^1(3|4|5|7|8)\d{9}$/, n = !0, o = document.getElementById("loginTip");
    void 0 != l && l.addEventListener(g, a, !1);
    var p = document.getElementById("aw-list"), q = document.getElementById("username"), r = document.getElementById("user"), s = document.getElementById("sending"), t = !0;
    //获取所点击的id，并插入到弹窗发送按钮的data-id属性中
    $(".aw-tip a").click(function () {
        var gift_id = $(this).attr("data-id");
        $("#sending").attr("data-id",gift_id);
    });
    void 0 != q && q.addEventListener("input", function () {
        var a = q.value;
        a && (r.innerHTML = a)
    }, !1), void 0 != p && p.addEventListener(g, function (a) {
        a.target && "A" == a.target.nodeName && (a.target.classList.contains("disabled") ? (a.target.classList.contains("over") ? j.show("over") : (a.target.classList.contains("unavailable") ? j.show("unavailable") : j.show("remind"))) : (j.show("send"), void 0 != s && s.addEventListener(g, function () {
            if (!t)return !1;
            var b = r.innerHTML;
            //获取传入data-id中的值
            var invite_id = $("#sending").attr("data-id");
            t = !1;
            var c = "send";
            $.ajax({
                type: "POST", dataType: "json", url: c, data: {name: b, id: invite_id}, success: function (b) {
                    j.hide("send"), a.target.classList.add("disabled")
                }, error: function () {
                }
            })
        }, !1)))
