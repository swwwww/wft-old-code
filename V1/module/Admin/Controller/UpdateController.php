...
class UpdateController extends BasisController
{
    use JsonResponse;
    ...
    public function indexAction()
    {
        ...
        $row = 10;
        $offset = ($page - 1) * $row;
        $res = $this->_getPlayClientUpdateTable()->fetchLimit($offset, $row, array(), array(), array('id' => 'desc'))->toArray();

        $count = $this->_getPlayClientUpdateTable()->fetchCount();

        //创建分页
        $url = '/wftadlogin/update/index';
        $paginator = new Paginator($page, $count, 20, $url);
    ...
    public function updateEditAction()
    {
        ...
        $res = $this->_getPlayClientUpdateTable()->get(array('id' => $id));

        $newfiledir = self::getUpdatefile($res->version_code);
        //移动文件

        return array('res' => $res, 'file' => file_exists($newfiledir['dir'] . '/' . $newfiledir['filename']));
    }
    ...
    public function reAction()
    {
        ...
        if ($status == 1) {
            $s = (int)$this->_getPlayClientUpdateTable()->update(array('status' => 1), array('id' => $id));
            return $this->_Goto('操作成功' . $s, '/wftadlogin/update/index');
        }
    }
    ...
    {
        $id = $this->getPost('id');
        $old_update = (int)$this->getPost('old_update');
        $version_info = $this->getPost('version_info');
        $all_update = $this->getQuery('all_update');
        ...
        if ($all_update) {
            //all update
            $first = $this->_getPlayClientUpdateTable()->fetchLimit(0, 1, array(), array('status' => 1), array('id' => 'desc'))->toArray();
            $first_id = (int)$first[0]['id'];
            $s = (int)$this->_getPlayClientUpdateTable()->update(array('old_update' => 1), "id!={$first_id}");
            return $this->_Goto('操作成功' . $s, '/wftadlogin/update/index');
        }

        if ($_FILES['file']['size']) {
            $appObj = new ApkParser();
            $targetFile = $_FILES['file']["tmp_name"];//apk所在的路径地址
            ...
            $s = $this->uploadUpdatefile($version_code);
            if (!$version_code or !$version_name or !$s) {
                exit('读取安装包或上传文件失败');
            }
        }

        if ($version_code) {
            $s = (int)$this->_getPlayClientUpdateTable()->get(array('version_code' => $version_code));
            if ($s) {
                exit('已存在此版本');
            }
        }
        ...
        if ($id) {
            //update
            $data = array('old_update' => $old_update, 'version_info' => $version_info);
            if ($version_code) {
            ...
            $s = (int)$this->_getPlayClientUpdateTable()->update($data, array('id' => $id));
            return $this->_Goto('操作成功' . $s, '/wftadlogin/update/index');
        } else {
            //insert
            if (!$_FILES['file']['size']) {
            ...
            $s = (int)$this->_getPlayClientUpdateTable()->insert(array('dateline' => time(), 'old_update' => $old_update, 'version_name' => $version_name, 'version_code' => $version_code, 'version_info' => $version_info));
            return $this->_Goto('操作成功' . $s, '/wftadlogin/update/index');
        }
    }
    ...
    {
        $id = (int)$this->getQuery('id');
        $s = $this->_getPlayClientUpdateTable()->update(array('status' => 0), array('id' => $id));
        return $this->_Goto('操作成功' . $s, '/wftadlogin/update/index');
    }

    public function uploadUpdatefile($version_code)
    {
        if (isset($_FILES['file'])) {
            $file = $_FILES['file'];
            //存放目录
            $newfiledir = self::getUpdatefile($version_code);

            self::createDir($newfiledir['dir']);
            //移动文件
            $res = move_uploaded_file($file["tmp_name"], $newfiledir['dir'] . '/' . $newfiledir['filename']);
            if ($res) {
                return true;
            } else {
                return false;
    ...
    //设置升级包目录
    public static function getUpdatefile($version_code)
    {
        //相对目录
        $dir = "/download";
        $filename = "wft-{$version_code}.apk";
    ...
    {
        $page = $this->getQuery('p', 1);
        $view = $this->getQuery('view');

        $where = array();
        if ($view) {
            $where = array('client_type' => $view);
        }
        $row = 10;
        $offset = ($page - 1) * $row;
        $res = $this->_getPlayPatchUpdateTable()->fetchLimit($offset, $row, array(), $where, array('id' => 'desc'))->toArray();

        $count = $this->_getPlayPatchUpdateTable()->fetchCount();

        //创建分页
        $url = '/wftadlogin/update/patchList';
        $paginator = new Paginator($page, $count, 20, $url);
    ...
    //修改升级数据
    public function patchEditAction()
    {
        $id = (int)$this->getQuery('id');
        $res = $this->_getPlayPatchUpdateTable()->get(array('id' => $id));

        $file = self::getPatchfile($res->client_type, $res->version_name, $res->patch_code);
        return array('res' => $res, 'file' => file_exists($file['dir'] . '/' . $file['filename']));
    }

    //恢复
    public function patchreAction()
    {
        $id = $this->getQuery('id', 0);
        $status = $this->getQuery('status', 0);

        if ($status == 1) {
            $s = (int)$this->_getPlayPatchUpdateTable()->update(array('status' => 1), array('id' => $id));
            return $this->_Goto('操作成功' . $s, '/wftadlogin/update/patchList');
        }
    }

    //保存升级
    public function patchSaveAction()
    {
        $id = $this->getPost('id');
        $version_name = $this->getPost('version_name');
        $patch_code = $this->getPost('patch_code');
        $client_type = $this->getPost('client_type');
        $version_info = $this->getPost('version_info');
        $hash_code = $this->getPost('hash_code');

        if ($id) {
            //update

            if ($_FILES['file']['size']) {
                $s = $this->uploadPatchfile($patch_code, $client_type, $version_name);
                if (!$s) {
                    exit('保存文件失败');
                }
            }
            $s = (int)$this->_getPlayPatchUpdateTable()->update(array('status' => 1, 'hash_code' => $hash_code, 'patch_code' => $patch_code, 'version_name' => $version_name, 'version_info' => $version_info, 'client_type' => $client_type), array('id' => $id));
            return $this->_Goto('操作成功' . $s, '/wftadlogin/update/patchList');
        } else {
            //insert
            if ($_FILES['file']['size']) {
            ...
            $s = (int)$this->_getPlayPatchUpdateTable()->insert(array('dateline' => time(), 'status' => 1, 'hash_code' => $hash_code, 'patch_code' => $patch_code, 'version_name' => $version_name, 'version_info' => $version_info, 'client_type' => $client_type));
            return $this->_Goto('操作成功' . $s, '/wftadlogin/update/patchList');
        }
    }

    public function patchDelAction()
    {
        $id = (int)$this->getQuery('id');
        $s = $this->_getPlayPatchUpdateTable()->update(array('status' => 0), array('id' => $id));
        return $this->_Goto('操作成功' . $s, '/wftadlogin/update/patchList');
    }

    public function uploadPatchfile($patch_code, $client_type, $version_name)
    {
        if (isset($_FILES['file'])) {
            $file = $_FILES['file'];

            $patch_file = self::getPatchfile($client_type, $version_name, $patch_code);

            self::createDir($patch_file['dir']);

            //移动文件
            $res = move_uploaded_file($file["tmp_name"], $patch_file['dir'] . '/' . $patch_file['filename']);
            if ($res) {
                return true;
            } else {
                return false;
