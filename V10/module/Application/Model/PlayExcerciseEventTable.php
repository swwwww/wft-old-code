    ...
    public function getSession($id)
    {
        return $this->tableGateway->select(function (Select $select) use ($id) {
            $select->columns(array(
                ...
                'vir_ault',
                'vir_child',
                'low_price' =>new Expression("(select min(price) from play_excercise_price   where `play_excercise_price`.`is_close` = 0 and  `play_excercise_price`.`eid` = `play_excercise_event`.`id`)")
            ));
...
//            $select->join('play_excercise_price', 'play_excercise_price.eid=play_excercise_event.id', array('low_price'=>new Expression('min(price)')), 'left');
     ...
     */
    public function getEventByCash($id,$limit=1)
    {
        if(!$id){
            $id = 0;
        }
        $event = $this->tableGateway->select(function (Select $select) use ($id,$limit) {
            $select->columns(array(
                'eid' => 'id',
                'bid',
                'start_time',
    ...
    public function session($bid){
        ...
        $data= $db->query("select count(*) as c from play_excercise_event WHERE bid=? AND sell_status>=1 
AND sell_status != 3
AND customize = 0
AND join_number< perfect_number
AND over_time >UNIX_TIMESTAMP()",array($bid))->current();
        if($data->c){
            return (int)$data->c;
        }else{
            return 0;
